...
class Category extends CI_Controller{
	...
	public function save_category(){
		$this->form_validation->set_rules('category_name','catégorie Name','required|min_length[2]');
		if($this->form_validation->run()){
		$this->CategoryModel->add_category_model();
	$this->session->set_flashdata("flsh_msg","<font class='success'>catégorie Ajoutée Succès</font>");
	...
	$this->session->set_flashdata("flsh_msg","<font class='success'>sous catégorie Ajoutée Succès</font>");
	...
	public function delete_category($category_id){
		...
		$this->session->set_flashdata("flsh_msg","<font class='success'>catégorie supprimée avec Succès</font>");
           redirect('category-list');
	}
	public function delete_sub_category($sub_category_id){
		$this->CategoryModel->delete_sub_caegory_by_id($sub_category_id);
		$this->session->set_flashdata("flsh_msg","<font class='success'>Sous Categorie supprimée avec Succès</font>");
           redirect('sub-category-list');
	}
	...
	public function update_category($category_id){
		$this->form_validation->set_rules('category_name','Nom catégorie','required|min_length[2]');
		if($this->form_validation->run()){
		$this->CategoryModel->update_caegory_by_id($category_id);
		$this->session->set_flashdata('flsh_msg','catégorie Modifiée avec Succès',10);
	...
	public function update_sub_category($sub_category_id){
		$this->form_validation->set_rules('sub_category_name','Nom sous catégorie ','required|min_length[2]');
		if($this->form_validation->run()){
		$this->CategoryModel->update_sub_caegory_by_id($sub_category_id);
		$this->session->set_flashdata('flsh_msg','sous catégorie Modifiée avec Succès',10);
